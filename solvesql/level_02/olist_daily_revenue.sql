...
olist_order_payments_dataset 테이블에는 주문 ID, 결제 방법, 결제 금액 등 각 주문의 결제와 관련된 정보가 저장되어 있습니다.
두 테이블을 이용해 2018년 1월 1일 이후 쇼핑몰의 일일 매출액을 계산하는 쿼리를 작성해주세요.
...
쿼리 결과는 아래 두 컬럼을 포함해야 하고, 매출 날짜 기준으로 오름차순 정렬되어 있어야 합니다.
매출액은 반올림 해 소수점 둘째자리까지 출력해주세요.

- dt - 매출 날짜 (예: 2018-01-01)
- revenue_daily - 해당 날짜의 매출액
*/

SELECT DATE(od.order_purchase_timestamp) AS dt, SUM(payment_value) AS revenue_daily
FROM olist_orders_dataset AS od
INNER JOIN olist_order_payments_dataset AS opd
