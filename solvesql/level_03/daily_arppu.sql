...
SELECT DATE(od.order_purchase_timestamp) AS dt,
    COUNT(DISTINCT od.customer_id) AS pu,
    ROUND(SUM(opd.payment_value), 2) AS revenue_daily,
    ROUND(SUM(opd.payment_value) / COUNT(DISTINCT od.customer_id), 2) AS arppu
